...
ICudaEngine *create_alexnet_engine(int max_batch_size, IBuilder *builder,
  ...
  IConvolutionLayer *conv1 = model->addConvolutionNd(
          *data, 64, DimsHW{11, 11}, weights["features.0.weight"],
  ...
  IPoolingLayer *pool1 = model->addPoolingNd(
          *relu1->getOutput(0), PoolingType::kAVERAGE, DimsHW{3, 3});
  assert(pool1);
  pool1->setStrideNd(DimsHW{2, 2});

  // Add convolution layer with 192 outputs and a 5x5 filter.
  IConvolutionLayer *conv2 = model->addConvolutionNd(
          *pool1->getOutput(0), 192, DimsHW{5, 5}, weights["features.3.weight"],
  ...
  IPoolingLayer *pool2 = model->addPoolingNd(
          *relu2->getOutput(0), PoolingType::kAVERAGE, DimsHW{3, 3});
  assert(pool2);
  pool2->setStrideNd(DimsHW{2, 2});

  // Add convolution layer with 384 outputs and a 3x3 filter.
  IConvolutionLayer *conv3 = model->addConvolutionNd(
          *pool2->getOutput(0), 384, DimsHW{3, 3}, weights["features.6.weight"],
          weights["features.6.bias"]);
  assert(conv3);
  conv3->setPaddingNd(DimsHW{1, 1});
  // Add activation layer using the ReLU algorithm.
  IActivationLayer *relu3 =
          model->addActivation(*conv3->getOutput(0), ActivationType::kRELU);
  assert(relu3);

  // Add convolution layer with 256 outputs and a 3x3 filter.
  IConvolutionLayer *conv4 = model->addConvolutionNd(
          *relu3->getOutput(0), 256, DimsHW{3, 3}, weights["features.8.weight"],
          weights["features.8.bias"]);
  assert(conv4);
  conv4->setPaddingNd(DimsHW{1, 1});
  // Add activation layer using the ReLU algorithm.
  IActivationLayer *relu4 =
          model->addActivation(*conv4->getOutput(0), ActivationType::kRELU);
  assert(relu4);

  // Add convolution layer with 256 outputs and a 3x3 filter.
  IConvolutionLayer *conv5 = model->addConvolutionNd(
          *relu4->getOutput(0), 256, DimsHW{3, 3},
          weights["features.10.weight"], weights["features.10.bias"]);
  assert(conv5);
  conv5->setPaddingNd(DimsHW{1, 1});
  // Add activation layer using the ReLU algorithm.
  IActivationLayer *relu5 =
          model->addActivation(*conv5->getOutput(0), ActivationType::kRELU);
  assert(relu5);

  // Add max pooling layer with stride of 3x3 and kernel size of 2x2.
  IPoolingLayer *pool3 = model->addPoolingNd(
          *relu5->getOutput(0), PoolingType::kAVERAGE, DimsHW{3, 3});
  assert(pool3);
  pool3->setStrideNd(DimsHW{2, 2});

  // Add fully connected layer with 4096 outputs.
  IFullyConnectedLayer *fc1 = model->addFullyConnected(
          *pool3->getOutput(0), 4096, weights["classifier.1.weight"],
  ...
  IFullyConnectedLayer *fc2 = model->addFullyConnected(
          *relu6->getOutput(0), 4096, weights["classifier.4.weight"],
  ...
  IFullyConnectedLayer *fc3 = model->addFullyConnected(
          *relu7->getOutput(0), number_classes, weights["classifier.6.weight"],
          weights["classifier.6.bias"]);
  assert(fc3);
